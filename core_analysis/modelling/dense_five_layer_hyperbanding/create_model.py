...
def build_model(hp):
    ni_neurons = hp.Int(name='ni_neurons', min_value=32, max_value=128, step=32)
    ...
    ni_neurons_activation = hp.Choice(name='ni_neurons_activation', values=['relu', 'tanh'])
    nj_neurons_activation = hp.Choice(name='nj_neurons_activation', values=['relu', 'tanh'])
    ...
    ni_neurons_dropout_value = hp.Float('ni_neurons_dropout_value', min_value=0.1, max_value=0.8, step=0.1)
    ...
    ni_neurons_batch_normalisation_momentum = hp.Float(name='ni_neurons_batch_normalisation_momentum', min_value=0.1,
                                                       max_value=0.9, step=0.1)
    ni_neurons_batch_normalisation_epsilon = hp.Choice(name='ni_neurons_batch_normalisation_epsilon',
                                                       values=[1e-3, 1e-4])
    ...
    model = call_existing_code(ni_neurons, nj_neurons, nk_neurons, nl_neurons, ni_neurons_activation,
                               nj_neurons_activation, nk_neurons_activation, nl_neurons_activation,
                               ni_neurons_dropout_value, nj_neurons_dropout_value, nk_neurons_dropout_value,
                               nl_neurons_dropout_value,
                               ni_neurons_batch_normalisation_momentum, ni_neurons_batch_normalisation_epsilon,
...
def call_existing_code(ni_neurons, nj_neurons, nk_neurons, nl_neurons, ni_neurons_activation,
                       nj_neurons_activation, nk_neurons_activation, nl_neurons_activation,
                       ni_neurons_dropout_value, nj_neurons_dropout_value, nk_neurons_dropout_value,
                       nl_neurons_dropout_value,
                       ni_neurons_batch_normalisation_momentum, ni_neurons_batch_normalisation_epsilon,
    ...
    model.add(Dense(ni_neurons,
                    kernel_initializer=glorot_uniform_initializer(),
                    bias_initializer='zeros',
                    input_shape=(data.dim_x,)))
    model.add(Activation(ni_neurons_activation))
    model.add(Dropout(ni_neurons_dropout_value))
    model.add(BatchNormalization(momentum=ni_neurons_batch_normalisation_momentum,
                                     epsilon=ni_neurons_batch_normalisation_epsilon,
